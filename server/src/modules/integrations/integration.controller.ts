...
router.post('/oauth/:platform/callback', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (platform === 'ALLEGRO') tokenData = await exchangeAllegroCode(code); else if (platform === 'OLX') tokenData = await exchangeOlxCode(code); else return res.status(400).json({ error: 'Unsupported platform' });
    const integ = await prisma.userIntegration.create({ data: { userId, platform, accessToken: tokenData.accessToken, expiresAt: new Date(Date.now() + tokenData.expiresIn * 1000) } });
    res.json(integ);
